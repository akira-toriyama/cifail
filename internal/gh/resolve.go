...
func (c *Client) ResolveRun(t Target) (model.Run, error) {
	switch {
	case t.RunID != 0:
		var r apiRun
		if err := c.getJSON(c.repoPath(fmt.Sprintf("/actions/runs/%d", t.RunID)), &r); err != nil {
	...
	case t.PR != 0:
		var pr apiPull
		if err := c.getJSON(c.repoPath(fmt.Sprintf("/pulls/%d", t.PR)), &pr); err != nil {
			return model.Run{}, err
		}
		if pr.Head.SHA == "" {
			return model.Run{}, core.APIf("PR #%d has no head SHA", t.PR)
		}
		return c.latestFailure(url.Values{"head_sha": {pr.Head.SHA}}, fmt.Sprintf("PR #%d", t.PR))

	case t.Branch != "":
		return c.latestFailure(url.Values{"branch": {t.Branch}}, fmt.Sprintf("branch %q", t.Branch))
...
func (c *Client) latestFailure(filter url.Values, desc string) (model.Run, error) {
	filter.Set("per_page", "30")
	var list apiRunList
	if err := c.getJSON(c.repoPath("/actions/runs?"+filter.Encode()), &list); err != nil {
		return model.Run{}, err
	}
